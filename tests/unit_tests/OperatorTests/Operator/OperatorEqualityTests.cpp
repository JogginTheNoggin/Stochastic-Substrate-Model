...
protected:
    ...
    std::unique_ptr<MockOperator> createOp(uint32_t id) {
        return std::make_unique<MockOperator>(id);
    }
};
...
TEST_F(OperatorEqualityTest, Equals_SameIdNoConnections) {
    std::unique_ptr<MockOperator> op1 = createOp(100); // Changed type
    std::unique_ptr<MockOperator> op2 = createOp(100); // Changed type
...
TEST_F(OperatorEqualityTest, Equals_DifferentIdNoConnections) {
    std::unique_ptr<MockOperator> op1 = createOp(101); // Changed type
    std::unique_ptr<MockOperator> op2 = createOp(102); // Changed type
...
TEST_F(OperatorEqualityTest, Equals_SameIdSameSingleConnection) {
    std::unique_ptr<MockOperator> op1 = createOp(103); // Changed type
    op1->addConnectionInternal(200, 1);
    std::unique_ptr<MockOperator> op2 = createOp(103); // Changed type
...
TEST_F(OperatorEqualityTest, Equals_SameIdDifferentSingleConnectionTarget) {
    std::unique_ptr<MockOperator> op1 = createOp(104); // Changed type
    op1->addConnectionInternal(200, 1);
    std::unique_ptr<MockOperator> op2 = createOp(104); // Changed type
...
TEST_F(OperatorEqualityTest, Equals_SameIdDifferentSingleConnectionDistance) {
    std::unique_ptr<MockOperator> op1 = createOp(105); // Changed type
    op1->addConnectionInternal(200, 1);
    std::unique_ptr<MockOperator> op2 = createOp(105); // Changed type
...
TEST_F(OperatorEqualityTest, Equals_SameIdMultipleConnectionsSameOrder) {
    std::unique_ptr<MockOperator> op1 = createOp(106); // Changed type
    op1->addConnectionInternal(200, 1);
    op1->addConnectionInternal(201, 1);
    op1->addConnectionInternal(202, 2);

    std::unique_ptr<MockOperator> op2 = createOp(106); // Changed type
    op2->addConnectionInternal(200, 1);
...
TEST_F(OperatorEqualityTest, Equals_SameIdMultipleConnectionsDifferentOrderInSet) {
    ...
    std::unique_ptr<MockOperator> op1 = createOp(107); // Changed type
    op1->addConnectionInternal(200, 1); // Target 200
    op1->addConnectionInternal(201, 1); // Target 201

    std::unique_ptr<MockOperator> op2 = createOp(107); // Changed type
...
TEST_F(OperatorEqualityTest, Equals_SameIdOneMissingConnection) {
    std::unique_ptr<MockOperator> op1 = createOp(108); // Changed type
    op1->addConnectionInternal(200, 1);
    op1->addConnectionInternal(201, 2);

    std::unique_ptr<MockOperator> op2 = createOp(108); // Changed type
...
TEST_F(OperatorEqualityTest, Equals_SameIdOneExtraConnection) {
    std::unique_ptr<MockOperator> op1 = createOp(109); // Changed type
    op1->addConnectionInternal(200, 1);

    std::unique_ptr<MockOperator> op2 = createOp(109); // Changed type
    op2->addConnectionInternal(200, 1);
...
TEST_F(OperatorEqualityTest, Equals_IdenticalTestOperatorsViaCreateOp) { // Renamed and logic adjusted
    ...
    std::unique_ptr<MockOperator> op1 = createOp(110);
    op1->addConnectionInternal(300, 1);

    std::unique_ptr<MockOperator> op2 = createOp(110); // Same ID
...
TEST_F(OperatorEqualityTest, Equals_IdenticalTestOperatorsWithConnections) { // Renamed and logic adjusted
    std::unique_ptr<MockOperator> op1 = std::make_unique<MockOperator>(111); // Direct creation
    op1->addConnectionInternal(300, 1);

    std::unique_ptr<MockOperator> op2 = std::make_unique<MockOperator>(111); // Direct creation, same ID
    op2->addConnectionInternal(300, 1); // Same connection
